...
class PPO:
    ...
    def __init__(self, policy_class, env, **hyperparameters):
        ...
        if policy_class in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new]:
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:                                                                       # ALG STEP 2
            ...
            if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new]:
                data_list = []
            ...
            if i_so_far % self.save_freq == 0:
                torch.save(self.actor.state_dict(), './dog2_env_optim_ppo_actor_RecurrentGCN_b.pth')
                torch.save(self.critic.state_dict(), './dog2_env_optim_ppo_critic_RecurrentGCN_b.pth')
    ...
    def get_action(self, obs):
        ...
        if self.policy_network in [GCN, GAT, rGIN, GatedGraphNN, GCN_mean,GCN_new,GConvN, RecurrentGCN, RecurrentGCN_new]:
    ...
    def evaluate(self, batch_obs, batch_acts):
        ...
        if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new]:
            ...
            # print(mean.reshape(batch_size,-1).shape)
            dist = MultivariateNormal(mean, self.cov_mat)
            ...
            #entropy_list = []
            #for i in range(batch_size):
            #    v_list.append(self.critic(batch_obs[i], self.edge_index).squeeze())
            #    mean = self.actor(batch_obs[i], self.edge_index)
            #    dist = MultivariateNormal(mean, self.cov_mat)
            #    log_probs_list.append(dist.log_prob(batch_acts[i]))
            #    entropy_list.append(dist.entropy().squeeze())
            # print(v_list)
            # print(mean_list)
            #V = torch.tensor(v_list, dtype=torch.float,requires_grad=True)
            #log_probs = torch.tensor(log_probs_list, dtype=torch.float)
            #entropy_list = torch.tensor(entropy_list, dtype=torch.float,requires_grad=True)
            #return V, log_probs, entropy_list
            # mean = torch.tensor(mean_list, dtype=torch.float)  
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.writer = SummaryWriter(log_dir='logs/dog2_env_RecurrentGCN_b') # Save training log
