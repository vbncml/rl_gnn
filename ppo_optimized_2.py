...
import numpy as np
# import wandb
from network import FeedForwardNN, MLP, GCN, GAT, rGIN, GatedGraphNN, GCN_mean, GCN_new
import torch
...
class PPO:
    ...
    def __init__(self, policy_class, critic_class, env, **hyperparameters):
        ...
        self.critic_network = critic_class
        # Initialize optimizers for actor and critic
        
        if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean]:
        ...
        if self.critic_network in [GCN, GAT, GatedGraphNN, GCN_mean]:
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:                                                                       # ALG STEP 2
            ...
            if self.critic_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new]:
                data_list = []
            ...
            if i_so_far % self.save_freq == 0:
                torch.save(self.actor.state_dict(), './dog2_env_optim_ppo_actor_GCN_MLP_new_c.pth')
                torch.save(self.critic.state_dict(), './dog2_env_optim_ppo_critic_GCN_MLP_new_c.pth')
    ...
    def get_action(self, obs):
        ...
        if self.policy_network in [GCN, GAT, rGIN, GatedGraphNN, GCN_mean,GCN_new]:
    ...
    def evaluate(self, batch_obs, batch_acts):
        ...
        batch_size = batch_obs.shape[0]
        if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new]:
            data_list = []
            for i in range(batch_size):
                data_list.append(Data(x=batch_obs[i], edge_index=self.edge_index))
                # V = torch.tensor(v_list, dtype=torch.float, requires_grad=True)
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.writer = SummaryWriter(log_dir='logs/dog2_env_GCN_MLP_new_c') # Save training log
