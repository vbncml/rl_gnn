...
class DogEnv_v3(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        self,
        xml_file="/home/ai/Desktop/2023/fall23/thesis/ppo/custom_envs/envs/dog.xml",
        ctrl_cost_weight=0.5,
        use_contact_forces=True,
        contact_cost_weight=5e-4,
        healthy_reward=1.0,
        terminate_when_unhealthy=True,
        healthy_z_range=(0.17, 0.4),
        ...
        **kwargs
        ...
        observation_space = Box(
            low=-np.inf, high=np.inf, shape=(9,2), dtype=np.float64
        )
    ...
    def _get_obs(self):
        if self._use_contact_forces:
            contact_force = self.contact_forces.flat
        ...
        for node in self._graph_dict:  
            ...
            if node != "root":
                ...
                if self._exclude_current_positions_from_observation:
                    # Obs 1: joint's pos
                    node_feats.append(np.mean(self.data.joint(node).qpos[2:])) 
                    # Obs 2: joint's vel
