...
class Dog2Env_v2(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._graph_dict = {
            "rootx":["left_bthigh","left_fthigh","right_bthigh","right_fthigh"],
            "rooty":["left_bthigh","left_fthigh","right_bthigh","right_fthigh"],
            "rootz":["left_bthigh","left_fthigh","right_bthigh","right_fthigh"],
        ...
        if exclude_current_positions_from_observation:
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(14,2,), dtype=np.float64
            )
        else:
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(15,2,), dtype=np.float64
            )

        MujocoEnv.__init__(
            self, "/home/ai/Desktop/2023/fall23/thesis/ppo/custom_envs/envs/dog_2.xml", 5, observation_space=observation_space, **kwargs
        )
    ...
    def _get_obs(self):
        all_node_feats = []
        #root_feats = []
        #root_feats.append(self.data.joint("rootx").qpos)
        #root_feats.append(self.data.joint("rooty").qpos)
        #rooty_obs = [self.data.joint("rooty").qpos, self.data.joint("rooty").qvel]
        #rootz_obs = [self.data.joint("rootz").qpos, self.data.joint("rootz").qvel]
        #print(root_feats)
