...
import numpy as np
# import wandb
from network import FeedForwardNN, MLP, GCN, GAT, rGIN, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new, AttentionSTGCN, RecurrentTGCN
import torch
...
class PPO:
    ...
    """
    def __init__(self, policy_class, env, transfer_env=None, **hyperparameters):
        """
        ...
        self.nodes_dim = env.observation_space.shape[0]

        #self.obs_dim = env.observation_space.shape[0]
        self.act_dim = env.action_space.shape[0]

        if self.transfer_env is not None:
            self.transfer_obs_dim = transfer_env.observation_space.shape[1]
            self.transfer_nodes_dim = transfer_env.observation_space.shape[0]
            self.transfer_act_dim = transfer_env.action_space.shape[0]

        if policy_class in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new, RecurrentTGCN]:
        ...
        if self.transfer_learning:
            self.critic = policy_class(self.transfer_obs_dim, 1)
        else:
            self.critic = policy_class(self.obs_dim, 1)

        self.policy_network = policy_class

        self.actor_path = './centipede2_env_optim_ppo3_actor_AttentionSTGCN_test.pth'
        self.critic_path = './centipede2_env_optim_ppo3_critic_AttentionSTGCN_test.pth'
        self.log_path = 'logs/centipede2_env_AttentionSTGCN_test'
        self.writer = SummaryWriter(log_dir=self.log_path)  # Save training log
        # Initialize optimizers for actor and critic
        # part where learning is frozen for GNNs
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps:                                                                       # ALG STEP 2
            ...
            if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new, RecurrentTGCN]:
                data_list = []
                ...
                if self.transfer_learning:
                    V = self.critic(batch.x, batch.edge_index, self.transfer_nodes_dim).squeeze()
                else:
                    V = self.critic(batch.x, batch.edge_index, self.nodes_dim).squeeze()
    ...
    def rollout(self):
        ...
        while t < self.timesteps_per_batch:
            ...
            for ep_t in range(self.max_timesteps_per_episode):
                ...
                if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new, RecurrentTGCN]:
                    # if self.transfer_learning:
                    #     val = self.critic(obs, self.edge_index, self.transfer_nodes_dim)
                    # else:
                    val = self.critic(obs, self.edge_index, self.nodes_dim)

                    val = torch.mean(val)
                else:
                    val = self.critic(obs)
                if self.transfer_learning:
                    obs, rew, terminated, truncated, _ = self.transfer_env.step(action)
                else:
    ...
    def get_action(self, obs):
        ...
        if self.policy_network in [GCN, GAT, rGIN, GatedGraphNN, GCN_mean,GCN_new,GConvN, RecurrentGCN, RecurrentGCN_new, RecurrentTGCN]:
            mean = self.actor(obs, self.edge_index, self.nodes_dim)
        else:
            mean = self.actor(obs)
    ...
    def evaluate(self, batch_obs, batch_acts):
        ...
        if self.policy_network in [GCN, GAT, GatedGraphNN, GCN_mean, GCN_new, GConvN, RecurrentGCN, RecurrentGCN_new, RecurrentTGCN]:
            ...
            mean = self.actor(batch.x, batch.edge_index, self.nodes_dim)
    ...
    def _init_hyperparameters(self, hyperparameters):
        ...
        self.transfer_learning = False                  # Sets the transfer learning mode on/off
